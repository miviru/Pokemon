...
public class PokemonController {
    ...
    public void lista10Primeros() { //FUNCIONA
        pokedex.pokemon.stream().limit(10).forEach(System.out::println);
    }

    public void ultimosCinco() {//FUNCIONA
...
//         Optional<Pokemon> a=pokedex.pokemon.stream().filter(c -> c.getName().toUpperCase().equalsIgnoreCase(nombre.toUpperCase())).findFirst();
         ...
         Optional<Pokemon> a2=pokedex.pokemon.stream().filter(c -> c.getName().equalsIgnoreCase(nombre.toUpperCase())).findFirst();
            return a2;
    }
...
//        System.out.println(pokedex.pokemon.stream().filter(pokemon -> pokemon.getName().length() > pokemon.getName().length()));
        System.out.println(pokedex.pokemon.stream().max((o1, o2) -> o1.getName().length() > o2.getName().length() ? -1 : 1));
    }
    ...
    public void debilidadMasComun(){//SUANDO "PREDICATE"O ALGO SIMILAR QUE HABIA EN LOS APUNTES DE GIT
        Map<List<String>, Long> agrupados=pokedex.pokemon.stream().collect(Collectors.groupingBy(Pokemon::getWeaknesses,Collectors.counting()));
        List<Map.Entry<List<String>, Long>> list = new ArrayList<>(agrupados.entrySet());
        list.sort(Map.Entry.comparingByValue());
        System.out.println(list.get(list.size()-1));

    }
    public List<Pokemon> globalPokemon(){
        return pokedex.pokemon.stream().collect(Collectors.toList());
    }
